...
def format_register(register: int) -> str:
    """Convert register number to human-readable name."""
    if register < 13:
        return f"R{register}"
    if register == 13:
        return f"%blockIdx"
...
def format_instruction(instruction: str) -> str:
    """Decode a 16-bit binary instruction string into assembly syntax."""
...
def format_core_state(core_state: str) -> str:
    """Convert core state bits to human-readable stage name."""
    core_state_map = {
        "000": "IDLE",
        "001": "FETCH",
        "010": "DECODE",
...
def format_fetcher_state(fetcher_state: str) -> str:
    """Convert fetcher state bits to human-readable name."""
    fetcher_state_map = {
        "000": "IDLE",
        "001": "FETCHING",
        "010": "FETCHED"
...
def format_lsu_state(lsu_state: str) -> str:
    """Convert LSU state bits to human-readable name."""
    lsu_state_map = {
        "00": "IDLE",
        "01": "REQUESTING",
        "10": "WAITING",
...
def format_memory_controller_state(controller_state: str) -> str:
    """Convert memory controller state bits to human-readable name."""
...
def format_registers(registers: List[str]) -> str:
    """Format all register values as a comma-separated string."""
    formatted_registers = []
...
def format_cycle(dut, cycle_id: int, thread_id: Optional[int] = None):
    """Log a complete execution trace for one clock cycle.
    
    Shows for each core and each thread:
      - Current instruction and PC
      - Core state (pipeline stage)
      - Fetcher and LSU states
      - All register values
      - ALU/LSU outputs
    """
    logger.debug(f"\n================================== Cycle {cycle_id} ==================================")
    ...
    for core in dut.cores:
        # Skip cores that don't have threads assigned
