...
class Memory:
    """Simulates external memory for the GPU testbench.
    
    Models both program memory (read-only, 16-bit) and data memory (read/write, 8-bit).
    Handles multi-channel concurrent access to match the GPU's memory interface.
    
    The GPU communicates with this simulated memory through:
      - Read: GPU sets mem_read_valid + mem_read_address → Memory returns mem_read_data + mem_read_ready
      - Write: GPU sets mem_write_valid + mem_write_address + mem_write_data → Memory sets mem_write_ready
    """
    def __init__(self, dut, addr_bits, data_bits, channels, name):
        self.dut = dut
        self.addr_bits = addr_bits
        self.data_bits = data_bits
        self.memory = [0] * (2**addr_bits)  # Initialize all memory to 0
        self.channels = channels
        self.name = name

        # Connect to DUT (Device Under Test) signals
        ...
        # Write signals only exist for data memory (not program memory)
        if name != "program":
    ...
    def run(self):
        """Process one cycle of memory requests.
        
        Called every clock cycle to:
        1. Check for pending read requests on each channel
        2. Return data for valid reads
        3. Process writes for data memory
        """
        # Parse read request signals
        mem_read_valid = [
        ...
        # Process write requests (data memory only)
    ...
    def load(self, rows: List[int]):
        """Load a list of values into memory starting at address 0."""
        for address, data in enumerate(rows):
            self.write(address, data)

    def display(self, rows, decimal=True):
        """Display memory contents as a formatted table."""
        logger.info("\n")
