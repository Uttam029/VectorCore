"""
Matrix Addition Kernel Test
============================
Tests the GPU by running a matrix addition kernel: C = A + B
where A and B are 1×8 matrices (vectors).
...
Data memory layout:
    [0-7]   = Matrix A: [0, 1, 2, 3, 4, 5, 6, 7]
    [8-15]  = Matrix B: [0, 1, 2, 3, 4, 5, 6, 7]
    [16-23] = Matrix C: [result written by kernel]

Expected result: C[i] = A[i] + B[i] = [0, 2, 4, 6, 8, 10, 12, 14]
"""
...
async def test_matadd(dut):
    # Program Memory - 13 instructions encoded as 16-bit binary
    ...
    # Data Memory - two 1×8 input matrices
    ...
    # Launch with 8 threads (one per element)
    threads = 8
